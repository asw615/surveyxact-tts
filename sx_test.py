...
# We use 'regex' for \p{L} and 'unicodedata' for normalization
import regex
import unicodedata
...
class TTSApp(ctk.CTk):
    ...
    def run_tts(self):
        ...
        def cleanup_for_tts(raw):
            ...
            # Normalize to NFC => unify codepoints for accented letters
        ...
        # Key function => letter-only, with NFC
        def letter_only_key(raw):
            # 1) Normalize to NFC
            s = unicodedata.normalize("NFC", raw)
            # 2) Lowercase
            s = s.lower()
            # 3) Remove non-letter chars
            s = regex.sub(r'[^\p{L}]', '', s)
            return s
